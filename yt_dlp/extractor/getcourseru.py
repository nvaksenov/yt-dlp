...
class GetCourseRuPlayerIE(InfoExtractor):
    _VALID_URL = r'https?://(?:player02\.getcourse\.ru|cf-api-2\.vhcdn\.com|player-ua\.gceuproxy\.com)/sign-player/?\?(?:[^#]+&)?json=[^#&]+'
...
class GetCourseRuIE(InfoExtractor):
    ...
    _BASE_URL_RE = rf'https?://(?:(?!player02\.)[^.]+\.getcourse\.(?:ru|io)|fedorova\.fun|{"|".join(map(re.escape, _DOMAINS))})'
    _VALID_URL = [
